...
const top = SettingsConst.BOARD_DIMENSIONS * SettingsConst.TILE_SIZE;
const bottom = 0;
const left = 0;
const right = SettingsConst.BOARD_DIMENSIONS * SettingsConst.TILE_SIZE;
...
export function renderChunkBorders(minX: number, maxX: number, minY: number, maxY: number, chunkSize: number, thickness: number): void {
   ...
   for (let chunkY = minY; chunkY <= maxY; chunkY++) {
      const screenY = chunkY * chunkSize * SettingsConst.TILE_SIZE;
      vertices.push(
   ...
   for (let chunkX = minX; chunkX <= maxX; chunkX++) {
      const screenX = chunkX * chunkSize * SettingsConst.TILE_SIZE;
      vertices.push(
